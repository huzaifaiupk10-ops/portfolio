...
function MarqueeRow({ cat, index, inView, isMobile }) {
  ...
  return (
    ...
    >
      ...
      {/* Category info */}
      <div style={{ padding: isMobile ? '1rem 0 0.5rem' : '2rem 2rem 2rem 0', position: 'relative', zIndex: 1 }}>
        ...
        <h3 style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: isMobile ? '1rem' : 'clamp(1rem,1.6vw,1.25rem)', color: paused ? 'var(--ink)' : 'var(--ink-2)', letterSpacing: '-0.01em', lineHeight: 1.2, transition: 'color 0.25s', marginBottom: '0.25rem' }}>{cat.title}</h3>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: 'var(--ink-3)', letterSpacing: '0.06em' }}>{cat.count} skills</p>
      </div>

      {/* Marquee */}
      <div style={{ overflow: 'hidden', position: 'relative', padding: isMobile ? '0.75rem 0 1rem' : '1.5rem 0' }}>
        ...
        >
          {repeated.map((skill, i) => (
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
              <span style={{ fontFamily: 'var(--font-body)', fontSize: isMobile ? '0.78rem' : '0.85rem', fontWeight: 500, color: paused ? 'var(--gold)' : 'var(--ink-2)', padding: isMobile ? '0.3rem 0.8rem' : '0.4rem 1.1rem', border: `1px solid ${paused ? 'rgba(201,168,76,0.35)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 999, background: paused ? 'rgba(201,168,76,0.07)' : 'rgba(255,255,255,0.02)', transition: 'color 0.3s, border-color 0.3s, background 0.3s', whiteSpace: 'nowrap' }}>
                {skill}
              </span>
    ...
    <section id="skills" style={{ padding: '7rem var(--gutter)', borderTop: '1px solid rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div ref={ref} style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '4rem', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            {['My', 'Expertise.'].map((word, i) => (
              <div key={word} style={{ overflow: 'hidden' }}>
                <motion.div
                  ...
                  className="metallic"
                  style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: isMobile ? 'clamp(2.5rem,10vw,4rem)' : 'clamp(3rem,7vw,5.5rem)', letterSpacing: '-0.03em', lineHeight: 0.9, paddingBottom: '0.06em' }}
                >
                  {word}
                </motion.div>
              </div>
          ...
          >
            <span className="metallic" style={{ fontFamily: 'var(--font-serif)', fontWeight: 700, fontSize: isMobile ? '2.5rem' : '3.5rem', letterSpacing: '-0.04em', lineHeight: 1 }}>{TOTAL}</span>
        ...
        <div>
          {CATEGORIES.map((cat, i) => (
            <MarqueeRow key={cat.num} cat={cat} index={i} inView={inView} isMobile={isMobile} />
          ))}
        ...
        <motion.div
          ...
          style={{ display: 'flex', gap: isMobile ? '1rem' : '2rem', marginTop: '3rem', flexWrap: 'wrap' }}
        >
          {CATEGORIES.map(cat => (
