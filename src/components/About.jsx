        ...
        <div>
          {['The person','behind the work.'].map((line,i)=>(
            <div key={i} style={{ overflow:'hidden' }}>
              <motion.div initial={{y:'105%'}} animate={inView?{y:'0%'}:{}} transition={{delay:i*0.12,duration:0.9,ease}} className="metallic" style={{ fontFamily:'var(--font-serif)', fontWeight:700, fontSize:'clamp(2.2rem,4.5vw,3.5rem)', letterSpacing:'-0.02em', lineHeight:0.95, paddingBottom:'0.06em' }}>{line}</motion.div>
            </div>
          ))}
