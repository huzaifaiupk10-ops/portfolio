  ...
  return (
    <section id="hero" style={{ minHeight:'100dvh', display:'flex', alignItems:'center', background:'var(--bg)', position:'relative', overflow:'hidden', paddingTop:64 }}>
      ...
      <div style={{ flex:'0 0 52%', maxWidth:'52%', padding:'4rem var(--gutter) 4rem clamp(1.25rem,6vw,6rem)', position:'relative', zIndex:2 }}>
        ...
        <motion.p initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:1.5,duration:0.5,ease}} style={{ fontFamily:'var(--font-serif)', fontWeight:500, fontSize:'1.2rem', color:'var(--gold)', marginBottom:'0.9rem', letterSpacing:'0.02em' }}>
          Hello, I'm
        </motion.p>
          ...
          {['Huzaifa','Imran'].map((w,i) => (
            <div key={w} style={{ overflow:'hidden' }}>
              <motion.div
                ...
                className="metallic"
                style={{ fontFamily:'var(--font-serif)', fontWeight:700, fontSize:'clamp(3.8rem,8.5vw,7rem)', letterSpacing:'-0.03em', lineHeight:0.88, paddingBottom:'0.07em', display:'block' }}
              >{w}</motion.div>
            </div>
          ))}
        </div>
