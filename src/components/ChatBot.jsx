...
function parseInline(text, onNavigate) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={i} style={{ color: '#F8F4EE', fontWeight: 600 }}>{part.slice(2, -2)}</strong>;
    }
    if (part.startsWith('[[') && part.endsWith(']]')) {
      ...
      return (
        <button key={i} onClick={() => onNavigate(url)}
          style={{ color: '#C8956A', textDecoration: 'underline', cursor: 'pointer', background: 'none', border: 'none', padding: 0, font: 'inherit', display: 'inline' }}>
          {label} →
        </button>
      );
    }
...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-1 px-4 py-3 rounded-2xl rounded-bl-sm w-fit"
      style={{ background: 'rgba(1,6,15,0.85)', border: '1px solid rgba(200,149,106,0.14)' }}>
      {[0, 1, 2].map((i) => (
        <motion.span key={i} className="w-1.5 h-1.5 rounded-full" style={{ background: '#C8956A' }}
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
      ...
      {!isUser && (
        <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold mr-2 flex-shrink-0 mt-0.5"
          style={{ background: 'linear-gradient(135deg,#9A6840,#C8956A)', color: '#00030A' }}>H</div>
      )}
      <div className="max-w-[80%] px-4 py-2.5 text-sm leading-relaxed"
        style={isUser
          ? { background: 'linear-gradient(135deg,#9A6840,#C8956A)', color: '#00030A', borderRadius: '1rem 1rem 0.25rem 1rem', fontWeight: 500 }
          : { background: 'rgba(1,6,15,0.85)', border: '1px solid rgba(200,149,106,0.14)', color: '#E0B898', borderRadius: '1rem 1rem 1rem 0.25rem' }
        }>
  ...
  return (
    <>
        ...
        {open && (
          <motion.div
            ...
            style={{
              ...
              boxShadow: '0 30px 80px rgba(0,0,0,0.6), 0 0 40px rgba(200,149,106,0.06)',
              maxHeight: 'calc(100dvh - 110px)',
            }}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
              style={{ background: 'rgba(0,3,10,0.98)', borderBottom: '1px solid rgba(200,149,106,0.1)' }}>
              ...
              <div className="flex items-center gap-1">
                {booking && (
                  <button onClick={cancelBooking}
                    ...
                    style={{ color: '#7A8BA8', border: '1px solid rgba(200,149,106,0.12)' }}
                ...
                <button onClick={() => setOpen(false)}
                  ...
                  style={{ color: '#7A8BA8' }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(200,149,106,0.1)')}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3" style={{ minHeight: 0 }}>
                ...
                <div className="space-y-2 pt-1">
                  {SUGGESTIONS.map((s) => (
                    <button key={s} onClick={() => send(s)}
                      ...
                      style={{ background: 'rgba(200,149,106,0.07)', border: '1px solid rgba(200,149,106,0.18)', color: '#C8956A' }}
                      onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(200,149,106,0.14)')}
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'rgba(200,149,106,0.07)')}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              {/* Confirm / cancel after booking summary */}
              {isBookingDone && !loading && (
                <div className="flex gap-2 pt-1">
                  <motion.button whileTap={{ scale: 0.96 }} onClick={confirmBooking}
                    ...
                    style={{ background: 'linear-gradient(135deg,#9A6840,#C8956A)', color: '#00030A' }}
                  ...
                  <motion.button whileTap={{ scale: 0.96 }} onClick={cancelBooking}
                    ...
                    style={{ background: 'rgba(1,6,15,0.85)', border: '1px solid rgba(200,149,106,0.15)', color: '#7A8BA8' }}>
                    Edit
                  </motion.button>
                </div>
              )}
            ...
            {!booking && (
              <div className="px-3 pb-2 flex-shrink-0">
                <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} onClick={startBooking}
                  ...
                  style={{ background: 'rgba(200,149,106,0.08)', border: '1px solid rgba(200,149,106,0.22)', color: '#C8956A' }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(200,149,106,0.15)')}
                  onMouseLeave={(e) => (e.currentTarget.style.background = 'rgba(200,149,106,0.08)')}>
            ...
            {!isBookingDone && (
              <div className="flex items-center gap-2 px-3 py-3 flex-shrink-0"
                style={{ borderTop: '1px solid rgba(200,149,106,0.08)' }}>
                <input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)}
                  ...
                  style={{ background: 'rgba(1,6,15,0.85)', border: '1px solid rgba(200,149,106,0.14)', color: '#F8F4EE' }}
                  onFocus={(e) => (e.currentTarget.style.borderColor = 'rgba(200,149,106,0.4)')}
                  onBlur={(e) => (e.currentTarget.style.borderColor = 'rgba(200,149,106,0.14)')} />
                <motion.button whileTap={{ scale: 0.92 }} onClick={handleSend}
                  disabled={!input.trim() || loading}
                  className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all"
                  style={{
                    background: input.trim() && !loading ? 'linear-gradient(135deg,#9A6840,#C8956A)' : 'rgba(1,6,15,0.7)',
                    color: input.trim() && !loading ? '#00030A' : '#374151',
                    border: '1px solid rgba(200,149,106,0.14)',
                  }}>
      ...
      <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.94 }}
        ...
        style={{
          background: open ? 'rgba(0,3,10,0.95)' : 'linear-gradient(135deg,#9A6840,#C8956A)',
          border: open ? '1px solid rgba(200,149,106,0.35)' : 'none',
          boxShadow: open ? '0 8px 30px rgba(0,0,0,0.4)' : '0 8px 30px rgba(200,149,106,0.3)',
          color: open ? '#C8956A' : '#00030A',
